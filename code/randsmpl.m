function [samps,x] = randsmpl(p, n, values)
...
% Insert this just in case we have a distribution that doesn't sum to 1
%p = p/sum(p);
...
if verLessThan('matlab','8.5')
    ...
    x = cast(interp1(edges,1:length(edges),rand(n,1),'previous'),'double');
else
    % Code to run in MATLAB R2015a and later
    x = cast(discretize(rand(n,1), edges),'double');   
