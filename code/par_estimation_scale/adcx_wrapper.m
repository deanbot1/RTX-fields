...
% How do we understand the following line?
% R_conc comes from xspan in main parameter_estimation code
% Should 53.7946 come from the parameter table? 
...
for i = 1: length(R_conc)
    ...
    [T,~,~,LDH,~,~]= adcx(p.tf_mol,p.tf_et,p.nr_t_mol,p.nr_t_et,p.T0,p.E0toT0,p.Estar0,p.g,p.r,p.kexp,p.gamma,...
        p.CD20,p.CD16,R_conc(i),p.kon20,p.Kd20,p.kon16,p.Kd16,p.gamma_perf);
